...
class HistoricalAnalyzer:
    def __init__(self, umd_db_path):
        ...
        logger.info(f"Initializing HistoricalAnalyzer with database: {umd_db_path}")
        try:
            self.df = pd.read_csv(umd_db_path)
            self.df['event_date'] = pd.to_datetime(self.df['event_date'])
            logger.info(f"Successfully loaded database with {len(self.df)} records")
    ...
    def analyze_industry_risks(self, target_industry, target_naics, lookback_years=10):
        ...
        logger.info(f"Analyzing industry risks for {target_industry}")
        ...
        logger.info(f"Found {analysis['total_incidents']} incidents for {target_industry}")
        return analysis
    
    def calculate_risk_factors(self, target_industry, target_naics, company_size):
        """Calculate risk factors based on historical data"""
        logger.info(f"Calculating risk factors for {target_industry}")
        ...
        logger.info(f"Calculated risk factors: {result}")
        return result
    ...
    def get_similar_incidents(self, scenario_description, target_industry, limit=3):
        ...
        logger.info(f"Found {len(relevant_incidents)} similar incidents")
